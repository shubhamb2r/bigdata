...
public class RevTransactionsCleanupApp {
	...
	public static void main( String[] args) {
		...
		PCollection<TableRow> inputRows = pipeline.apply("Reading Data", BigQueryIO.readTableRows()
				.fromQuery(SELECT_ALL_REV_TRANSACTIONS).usingStandardSql());

		PCollection<TableRow> transformedRows 
			= inputRows.apply("Transforming Rows", ParDo.of(new RevTransactionsTrasformRowFn()));


		transformedRows.apply("Writing Output", 
				BigQueryIO.writeTableRows().to("focus-task-271908:ecommerce.rev_transactions_dataflow")
				.withSchema(RevTransactionsSchema.createSchema())
